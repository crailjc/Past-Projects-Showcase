...
void NeuralNet::learn(const Matrix& inputs, const Matrix& expected,
    ...
    for (size_t lyr = 0; (lyr < biases.size()); lyr++) {
        ...
        // CHANGES
        // weights[lyr].dot2(activation, biases[lyr], result);
        // zs.push_back(result);
    ...
    // Create intermediate bias and weights matrices to be updated asf 
...
NeuralNet::classify(const Matrix& inputs, Matrix& result) const {
    ...
    for (size_t lyr = 0; (lyr < weights.size()); lyr++) {
        // result = (weights[lyr+0].dot(result) + biases[lyr+0]).apply(sigmoid);
        // result = (weights[lyr+1].dot(result) + biases[lyr+1]).apply(sigmoid);
        // CHANGES

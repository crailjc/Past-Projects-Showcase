 ...
 * pixel in the image.
 */
// Matrix loadPGM(const std::string& path, Matrix img) {
void loadPGM(const std::string& path, Matrix& img) {
    // Make a static map so that the matrices read in are saved by their
    // path so that if they are needed again they can be got form the map
    static std::unordered_map<std::string, Matrix> pathsMatrix;

    //  If the paths is already in the map set img to 
    // that matrix and return 
    if (pathsMatrix.find(path) != pathsMatrix.end()) {
        img = pathsMatrix[path];
        // return;
    // else do the processing as normal
    } else {
        ...
        // If the width and height are not the same from
        // the last matrix instantiate a new matrix for img
        ...
        // Add the path and matrix to the map for it to be used
        pathsMatrix[path] = img;
    }
}
...
void train(NeuralNet& net, const std::string& path,
           ...
           int count = 1e6) {
    // moved the img to outside of the loop
    Matrix img, exp;

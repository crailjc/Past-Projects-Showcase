...
dvec3 cameraPos2(9, 9, 9);
dvec3 cameraPos3(-9, 9, -9);
dvec3 cameraPos4(20, 4, 5);
dvec3 cameraFocus1 = ORIGIN3D;
dvec3 cameraFocus2 = ORIGIN3D;
dvec3 cameraFocus3 = dvec3(15,0,0);
dvec3 cameraFocus4 = dvec3(-5, -5, 5);
dvec3 cameraUp1 = Y_AXIS;
dvec3 cameraUp2 = Y_AXIS;
dvec3 cameraUp3 = Y_AXIS;
dvec3 cameraUp4 = Y_AXIS;
...
void render() {
	...
	if (multiViewOn) {
		// Determine the region for viewport n
		// Change viewing parameters of camera to match viewport n’s size
		// Change configuration of camera to vantage point n
		// Raytrace scene, you will need to pass viewport n region

		pCamera = PerspectiveCamera(cameraPos2, cameraFocus2, cameraUp2, cameraFOV, width/2, height/2);
		rayTrace.raytraceScene(frameBuffer, numReflections, scene, 0, width / 2, top/2, 0);

		pCamera = PerspectiveCamera(cameraPos3, cameraFocus3, cameraUp3, cameraFOV, width, height);
		rayTrace.raytraceScene(frameBuffer, numReflections, scene, width/2, right, top/2, 0);

		pCamera = PerspectiveCamera(cameraPos4, cameraFocus4, cameraUp4, cameraFOV, width, height);
		rayTrace.raytraceScene(frameBuffer, numReflections, scene, width / 2, right, top, top/2);
		
	} else {
		pCamera = PerspectiveCamera(cameraPos1, cameraFocus1, cameraUp1, cameraFOV, width, height);
		rayTrace.raytraceScene(frameBuffer, numReflections, scene, left, right, top, bottom);
	}
...
ISphere *sphere1 = new ISphere(dvec3(0, 2.0, 10), 2.0);
IClosedCylinderY* closedCylinder = new IClosedCylinderY(dvec3(5.0, 0.0, 0.0), 2.0, 5.0);
IConeY* cone = new IConeY(dvec3(-10.0, 0.0, 2.0), 2.0, 5.0);
ICylinderZ* cylinderZ = new ICylinderZ(dvec3(10, 0.0, -9.0), 2.0, 6.0);
IShapePtr cylinder1 = new ICylinderY(dvec3(-10, 2, 10), 3.0, 3.0);

void buildScene() {

	// Have to loop through a scene array/vector?
	...
	scene.addOpaqueObject(new VisibleIShape(cylinderZ, bronze));
	scene.addOpaqueObject(new VisibleIShape(cylinder1, gold, &im1));

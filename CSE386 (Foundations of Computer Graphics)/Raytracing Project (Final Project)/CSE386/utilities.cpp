 ...
 * 			That is, their values within  of each other.
...
double normalizeDegrees(double degrees) {
	...
	// If result is less than zero add 360 else no change
...
double normalizeRadians(double rads) {
	double tempdegs = rad2deg(rads);
	double result = deg2rad(normalizeDegrees(tempdegs));
...
double areaOfTriangle(double a, double b, double c) {
	double s = (a + b + c) / 2;
	// No value can be negative nor can s value be less than a, b, or c
	if ((a <= 0 || b <= 0 || c <= 0) || (s <= a || s <= b || s <= c)) {
		return -1;
	}
...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
	double side_a = distanceBetween(x1, y1, x2, y2);
	double side_b = distanceBetween(x2, y2, x3, y3);
	double side_c = distanceBetween(x3, y3, x1, y1);

	return areaOfTriangle(side_a, side_b, side_c);
}
/**
...
dvec2 pointOnCircle(const dvec2& center, double R, double angleRads) {
	// Get the original position of x and y and shift them over 
...
double directionInRadians(const dvec2& referencePt, const dvec2& targetPt) {
	return normalizeRadians(glm::atan(targetPt.y - referencePt.y, targetPt.x - referencePt.x));
}
...
double directionInRadians(double x1, double y1, double x2, double y2) {
	return normalizeRadians(glm::atan(y2 - y1, x2 - x1));
}
 ...
 * @param	pt1	The first point. A
 * @param	pt2	The second point. B
 * @param	pt3	The third point. C
 * @return	Area of triangle.
 */
...
double areaOfTriangle(const dvec3& pt1, const dvec3& pt2, const dvec3& pt3) {
	dvec3 AB = pt1 - pt2;
	dvec3 AC = pt1 - pt3;
	return .5 * areaOfParallelogram(AB, AC);
}
...
dvec3 pointingVector(const dvec3& pt1, const dvec3& pt2) {
	dvec3 unitVec = pt2 - pt1;
	return myNormalize(unitVec);
}
...
vector<double> quadratic(double A, double B, double C) {
	...
	if (!glm::isnan(root2) && root2 != root1) {
		result.push_back(root2);
	}
	if (result.size() == 2) {
		max = glm::max(result.at(0), result.at(1));
		min = glm::min(result.at(0), result.at(1));
		result.clear();
...
int quadratic(double A, double B, double C, double roots[2]) {
	...
	if (result.size() == 0) {
		...
		roots[0] = glm::min(result.at(0), result.at(1));
		roots[1] = glm::max(result.at(0), result.at(1));
		return 2;
	}
